...
skillsDict = {"Charisma": 0, "Intuition": 1, "Handiwork": 2, "Conventional Weapons": 3, "Unconventional Weapons": 4, "Exotic" : 5, "General Knowledge": 6, "Auxiliary": 7 }
statsDict = {"Energy": 0, "Durability": 1, "Maneuverability": 2, "Hacking Systems": 3, "Computer": 4, "PSI Unit": 5, "Robotics": 6, "Engines": 7}
...
def deleteSelected():
    ...
    if selected['values']!='':
        try: parenttype=tree.item(tree.parent(tree.focus()))['values'][0]
...
def updateTree():
    ...
    for key in allstuff:
        ...
        for playah in sorted(list(sorted(list(allstuff[key].values()),key=lambda t:t.name)),key=lambda t:t.extra):
            opened=True
            try: opened=collapsed[playah.name]
            except Exception: pass
...
def updateDetails(event):
    ...
    if selected['values']!='':
        ...
        if selected['values'][0]=='Item':
            ...
            renameb.config(command=lambda: EditPopup(root, parentobj.getitemfromid(tree.focus()).rename, "New name?", currentv=selected['text']))
            ammob = Button(detailbuttonfr, text='Change Ammo', command=lambda: EditPopup(root, parentobj.getitemfromid(tree.focus()).changeammo, "New ammo?", currentv=selected['values'][2]))
            ammob.pack(fill=BOTH, expand=1)
            ownb = Button(detailbuttonfr, text='Change Owner', command=lambda: OwnerPopup(root, parentobj.getitemfromid(tree.focus()), parentobj))
            ...
            renameb.config(command=lambda: EditPopup(root, parentobj.getmodfromname(selected['text']).rename, "New name?", currentv=selected['text']))
        elif selected['values'][0]=='Players':
            details.set(players[selected['text']].info())
            renameb.config(command=lambda: EditPopup(root, players[selected['text']].rename, "New name?", currentv=selected['text']))
            extb = Button(detailbuttonfr, text='Change Squad', command=lambda: EditPopup(root, players[selected['text']].changeextra, "New Squad?", currentv=selected['values'][1]))
            ...
            renameb.config(command=lambda: EditPopup(root, ships[selected['text']].rename, "New name?", currentv=selected['text']))
            extb = Button(detailbuttonfr, text='Change Squad', command=lambda: EditPopup(root, ships[selected['text']].changeextra, "New Squad?", currentv=selected['values'][1]))
            extb.pack(fill=BOTH, expand=1)
        elif selected['values'][0]=='Piles':
            details.set(piles[selected['text']].info())
            renameb.config(command=lambda: EditPopup(root, piles[selected['text']].rename, "New name?", currentv=selected['text']))
...
class ItemPopup():
    ...
    def __init__(self, parent):
        ...
        self.v = IntVar()
        try: self.v.set(types[selected['values'][0]])
        except Exception: self.v.set(1)
